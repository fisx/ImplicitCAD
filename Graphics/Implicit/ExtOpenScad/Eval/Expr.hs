...
import Graphics.Implicit.ExtOpenScad.Definitions
  ( Expr (LamE, ListE, LitE, Var, (:$)),
    Message (Message),
    MessageType (Error),
    OVal (OError, OFunc, OList, OUndefined),
...
import Graphics.Implicit.ExtOpenScad.Util.OVal (getErrors, oTypeStr)
...
import Prelude (Maybe (Just, Nothing), String, const, fmap, foldMap, foldr, pure, traverse, zip, (!!), ($), (.), (<$>), (<>))
...
evalExpr pos expr = do
  vars <- getVarLookup
  let (valf, s) = runState (evalExpr' expr) $ ExprState vars [] [] pos
