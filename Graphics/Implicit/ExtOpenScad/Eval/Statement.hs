...
import Graphics.Implicit.ExtOpenScad.Definitions
  ( CompState (CompState, messages, scadOpts, sourceDir),
    Expr (LitE),
    Message (Message),
    OVal (OBool, ONModule, OUModule, OVargsModule),
    Pattern (Name),
    ScadOpts (importsAllowed),
    StateC,
    Statement (DoNothing, If, Include, ModuleCall, NewModule, (:=)),
...
import Graphics.Implicit.ExtOpenScad.Util.ArgParser (argMap, argument, defaultTo)
import Graphics.Implicit.ExtOpenScad.Util.OVal (getErrors)
import Graphics.Implicit.ExtOpenScad.Util.StateC (addMessage, errorC, getRelPath, getVals, getVarLookup, lookupVar, modifyVarLookup, pushVals, putVals, scadOptions, warnC, withPathShiftedBy)
import System.Directory (doesFileExist)
import System.FilePath (takeDirectory)
import Prelude (Bool (False, True), Either (Left, Right), Maybe (Just, Nothing), String, elem, filter, fmap, fst, init, last, length, not, notElem, null, pure, readFile, reverse, show, snd, traverse, zip, ($), (&&), (*>), (.), (/=), (<$), (<$>), (<>), (==))
...
runStatementI (StatementI sourcePos (NewModule name argTemplate suite)) = do
  ...
  argNames <- for argTemplate $ \(argName, defexpr) -> do
    defval <- traverse (evalExpr sourcePos) defexpr
    let hasDefault = isJust defval
    pure (argName, hasDefault)
  runStatementI . StatementI sourcePos $
    (Name name :=) $
      LitE $
        OUModule name (Just argNames) $ \(VarLookup varlookup) -> do
          newNameVals <- for argTemplate' $ \(argName, maybeDef) -> do
            ...
            pure (argName, val)
          let varlookup' = union (fromList newNameVals) varlookup
...
runStatementI (StatementI sourcePos (ModuleCall (Symbol name) argsExpr suite)) = do
  ...
  newVals <- case maybeMod of
    Just (OUModule _ args mod') -> do
      ...
      -- Run the module.
      let argsMapped = argMap evaluatedArgs $ mod' varLookup
    ...
    Just (ONModule _ implementation forms) -> do
      ...
      suiteResults <- case suiteInfo of
        ...
        _ -> do
          ...
          pure []
      -- Run the module.
      let argsMapped = argMap evaluatedArgs $ implementation sourcePos suiteResults
      for_ (pack <$> snd argsMapped) $ errorC sourcePos
      fromMaybe (pure []) (fst argsMapped)
  ...
  where
    ...
    checkOptions args makeWarnings = do
      let -- Find what arguments are satisfied by a default value, were given in a named parameter, or were given.. and count them.
          ...
          parameterReport =
            "Passed "
              ...
              <> ( if not (null missingNotDefaultable)
                     then
      ...
      when (makeWarnings)
        (errorC sourcePos $ "missingNotDefaultable: " <> show (length missingNotDefaultable))
         -}
      when
        (not (null missingNotDefaultable) && makeWarnings)
        (errorC sourcePos $ "Insufficient parameters. " <> pack parameterReport)
      when
        (not (null extraUnnamed) && isJust args && makeWarnings)
