...
import Data.Text.Lazy (Text, intercalate, length, pack, singleton, unpack)
import qualified Data.Text.Lazy as TL (index)
import Graphics.Implicit.Definitions (ℕ, ℝ)
import Graphics.Implicit.ExtOpenScad.Definitions (MessageType (TextOut, Warning), OVal (OBool, OError, OFunc, OList, ONum, OString, OUndefined, OVargsModule), ScadOpts (ScadOpts), SourcePosition, StateC, StatementI, Symbol (Symbol), VarLookup (VarLookup))
import Graphics.Implicit.ExtOpenScad.Primitives (primitiveModules)
import Graphics.Implicit.ExtOpenScad.Util.OVal (oTypeStr, toOObj)
import Graphics.Implicit.ExtOpenScad.Util.StateC (addMessage, modifyVarLookup, scadOptions)
import Prelude (Bool (False, True), Maybe (Just, Nothing), abs, acos, asin, atan, atan2, ceiling, cos, cosh, exp, flip, floor, fmap, foldMap, foldl, fromInteger, fromIntegral, id, log, max, min, mod, not, otherwise, pi, round, show, signum, sin, sinh, sqrt, tan, tanh, zipWith, ($), (&&), (*), (**), (+), (-), (.), (/), (/=), (<), (<$>), (<=), (<>), (==), (>), (>=), (||))

defaultObjects :: Bool -> VarLookup
defaultObjects withCSG =
  VarLookup $
    fromList $
      defaultConstants
        <> defaultFunctions
...
varArgModules =
  ...
  where
    ...
    echo _ pos args suite runSuite = do
      ...
      let text :: [(Maybe Symbol, OVal)] -> Text
...
defaultPolymorphicFunctions =
  ...
  where
    ...
    index (OList l) (ONum ind) =
      let n :: ℕ
          n = floor ind
       in if n < genericLength l then l `genericIndex` n else OError "List accessed out of bounds"
