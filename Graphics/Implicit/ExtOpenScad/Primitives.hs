...
import Graphics.Implicit.Definitions (ExtrudeMScale (C1), SymbolicObj2, SymbolicObj3, fromℕtoℝ, isScaleID, ℕ, ℝ, ℝ2, ℝ3)
import Graphics.Implicit.ExtOpenScad.Definitions (ArgParser (APFail), OVal (ONModule, OObj2, OObj3), SourcePosition, StateC, Symbol (Symbol))
import Graphics.Implicit.ExtOpenScad.Util.ArgParser (defaultTo, doc, eulerCharacteristic, example, test)
...
import qualified Graphics.Implicit.Primitives as Prim (circle, cylinder2, difference, differenceR, extrude, extrudeM, intersect, intersectR, mirror, pack2, pack3, polygon, rect, rect3, rotate, rotate3, rotate3V, rotateExtrude, scale, shell, sphere, translate, union, unionR, withRounding)
import Linear (V2 (V2), V3 (V3))
import Linear.Affine (qdA)
import Prelude (Bool (False, True), Either (Left, Right), Maybe (Just, Nothing), const, cos, either, error, fmap, id, not, null, otherwise, pi, pure, sin, uncurry, ($), (&&), (*), (-), (.), (/), (/=), (<), (<>), (==), (>), (||))
...
primitiveModules =
  [ onModIze sphere [([("r", noDefault)], noSuite), ([("d", noDefault)], noSuite)],
    ...
    onModIze
      cylinder
      [ ([("r", hasDefault), ("h", hasDefault), ("r1", hasDefault), ("r2", hasDefault), ("$fn", hasDefault), ("center", hasDefault)], noSuite),
...
cylinder = moduleWithoutSuite "cylinder" $ \_ _ -> do
  ...
  test "cylinder(r=5, h=10, $fn = 6);"
    `eulerCharacteristic` 0
  let V2 h1 h2 = either (toInterval center) id h
      dh = h2 - h1
  ...
  let addPolyOrSquare pts
        | [p1, p2, p3, p4] <- pts =
          let d1d2 = qdA p1 p2
           ...
           in -- Rectangles have no overlapping points,
...
scale = moduleWithSuite "scale" $ \_ children -> do
  ...
  let scaleObjs stretch2 stretch3 =
...
extrude = moduleWithSuite "linear_extrude" $ \_ children -> do
  ...
  height :: Either ℝ (ℝ -> ℝ -> ℝ) <-
    argument "height" `defaultTo` Left 1
      `doc` "height to extrude to..."
  center :: Bool <-
    argument "center" `defaultTo` False
      `doc` "center? (the z component)"
  twistArg :: Either ℝ (ℝ -> ℝ) <-
    argument "twist" `defaultTo` Left 0
      `doc` "twist as we extrude, either a total amount to twist or a function..."
  scaleArg :: ExtrudeMScale <-
    argument "scale" `defaultTo` C1 1
      `doc` "scale according to this funciton as we extrude..."
  translateArg :: Either ℝ2 (ℝ -> ℝ2) <-
    argument "translate" `defaultTo` Left (pure 0)
      `doc` "translate according to this funciton as we extrude..."
  r :: ℝ <-
    argument "r" `defaultTo` 0
      `doc` "round the top/bottom."
  let heightn = case height of
        Left h -> h
        Right f -> f 0 0
  ...
  pure $
    pure $
      obj2UpMap
        ( \obj -> case height of
            Left constHeight
              | isTwistID && isScaleID scaleArg && isTransID ->
