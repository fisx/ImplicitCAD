...
import Data.Map (delete, fromList, lookup)
import qualified Data.Map as DM (null)
import Data.Maybe (fromJust, isJust, isNothing)
import Data.Text.Lazy (Text, pack, unpack)
import Graphics.Implicit.Definitions (ℕ)
import Graphics.Implicit.ExtOpenScad.Definitions (ArgParser (AP, APBranch, APExample, APFail, APTerminator, APTest), OVal (OError), Symbol, TestInvariant (EulerCharacteristic), VarLookup (VarLookup))
import Graphics.Implicit.ExtOpenScad.Util.OVal (OTypeMirror, fromOObj, toOObj)
import Prelude (Maybe (Just, Nothing), String, filter, fmap, foldl1, fst, maybe, not, return, show, snd, ($), (&&), (.), (<$>), (<>))
...
argument name =
  AP name Nothing "" $ \oObjVal -> do
    let val :: Maybe desiredType
        val = fromOObj oObjVal
        errmsg :: Text
        errmsg = case oObjVal of
          OError err ->
            "error in computing value for argument " <> pack (show name)
              <> ": "
              <> err
...
argMap ::
  ...
  (Maybe a, [String])
argMap args = argMap2 unnamedArgs (VarLookup $ fromList namedArgs)
  where
