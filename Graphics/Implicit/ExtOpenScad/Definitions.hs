...
module Graphics.Implicit.ExtOpenScad.Definitions
  ( ArgParser (AP, APTest, APBranch, APTerminator, APFail, APExample),
    Symbol (Symbol),
...
import Control.Monad (Functor, Monad, MonadPlus, ap, mplus, mzero, (>=>), (>>=))
import Control.Monad.State (StateT)
import Data.Map (Map, lookup, union)
import Data.Maybe (fromMaybe)
import Data.Text.Lazy (Text, intercalate, unpack)
import Graphics.Implicit.Definitions (Fastℕ, SymbolicObj2, SymbolicObj3, fromFastℕ, ℕ, ℝ)
import Prelude (Bool (False), Eq, FilePath, IO, Int, Maybe (Just), Ord, Show, and, show, zipWith, ($), (<$>), (<>), (==))
...
data CompState = CompState
  { -- | A hash of variables and functions.
    ...
    sourceDir :: FilePath,
    -- | Output strings, warnings, and errors generated during execution.
    messages :: [Message],
    -- | Options controlling the execution of scad code.
    scadOpts :: ScadOpts
  }
...
data MessageType
  = TextOut -- text intetionally output by the ExtOpenScad program.
  | Warning
  | Error
  | SyntaxError
  | Compatibility
