...
import Graphics.Implicit.ExtOpenScad.Definitions (Expr (LamE, ListE, LitE, (:$)), OVal (ONum, OUndefined), Symbol (Symbol))
import qualified Graphics.Implicit.ExtOpenScad.Definitions as GIED (Expr (Var), Pattern (Name))
import Graphics.Implicit.ExtOpenScad.Parser.Lexer (matchAND, matchCAT, matchColon, matchComma, matchEQ, matchEXP, matchGE, matchGT, matchIdentifier, matchLE, matchLT, matchLet, matchNE, matchOR, matchTok, surroundedBy, whiteSpace)
import Graphics.Implicit.ExtOpenScad.Parser.Util (boolean, number, scadString, scadUndefined, variable, (*<|>), (?:))
import Text.Parsec (chainl1, chainr1, many, oneOf, option, optionMaybe, sepBy, (<|>))
import Text.Parsec.String (GenParser)
import Prelude (Char, Maybe (Just, Nothing), flip, foldl, foldr, head, id, length, pure, ($), (&&), (*>), (.), (<$>), (<*), (<*>), (<>), (==))
