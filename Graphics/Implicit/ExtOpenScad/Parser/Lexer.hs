...
import Text.Parsec.Token (GenTokenParser, caseSensitive, colon, comma, commentEnd, commentLine, commentStart, identLetter, identStart, makeTokenParser, nestedComments, reservedNames, reservedOpNames, semi)
import qualified Text.Parsec.Token as P (identifier, reserved, reservedOp, whiteSpace)
import Prelude (Bool (True), Char, String, not, pure, ($), (&&), (>>))
