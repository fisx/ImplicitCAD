...
import Graphics.Implicit.Definitions (Box2, ℝ, ℝ2, ℝ3)
import Linear (Metric, Quaternion (Quaternion), V2 (V2), V3 (V3), distance, dot, norm, normalize, (*^))
import Prelude (Bool, Fractional, Num, Ordering, RealFloat, abs, asin, atan2, compare, flip, fst, head, max, maximum, min, minimum, not, otherwise, pi, signum, sin, snd, sqrt, ($), (&&), (*), (+), (-), (.), (/), (<), (<=), (<>), (==), (>), (>=), (||))
...
box3sWithin r (V3 ax1 ay1 az1, V3 ax2 ay2 az2) (V3 bx1 by1 bz1, V3 bx2 by2 bz2) =
  let near (a1, a2) (b1, b2) = not $ (a2 + r < b1) || (b2 + r < a1)
   in (ax1, ax2) `near` (bx1, bx2)
...
rmaximum r l
  | r == 0 = maximum l
  | otherwise =
    let tops = sortBy (flip compare) l
     in rmax r (head tops) (tops !! 1)
      ...
      if abs (x2 - x1) <= abs (bx2 - bx1) && abs (y2 - y1) <= abs (by2 - by1)
        then
          let row =
           ...
           in rowAndUp
        else tmap2 (presObj :) $ packSome otherBoxedObjs box
    packSome [] _ = ([], [])
