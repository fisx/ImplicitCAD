...
import Graphics.Implicit.Definitions
  ( Box2,
    SharedObj (Complement, DifferenceR, IntersectR, UnionR),
    SymbolicObj2 (Circle, Polygon, Rotate2, Shared2, Square),
    minℝ,
    ℝ,
    ℝ2,
  )
import Graphics.Implicit.ObjectUtil.GetBoxShared (corners, emptyBox, getBoxShared, intersectBoxes, outsetBox, pointsBox, unionBoxes)
-- To construct vectors of ℝs.
import Linear (V2 (V2))
import Prelude (Eq, atan2, cos, error, fmap, max, maximum, min, minimum, otherwise, pi, pure, show, sin, sqrt, unzip, ($), (&&), (*), (+), (-), (/), (<), (<>), (==), (>), (||))
...
getBox2R (Polygon points) deg =
  let pointRBoxes = [pointRBox point deg | point <- points]
      (pointValsMin, pointValsMax) = unzip pointRBoxes
      unbox :: ℝ2 -> (ℝ, ℝ)
      unbox (V2 x y) = (x, y)
      (pointValsX, pointValsY) = unzip $ fmap unbox $ pointValsMin <> pointValsMax
   in (V2 (minimum pointValsX) (minimum pointValsY), V2 (maximum pointValsX) (maximum pointValsY))
...
getBox2R (Shared2 (IntersectR r symObjs)) deg =
  let boxes = [getBox2R obj deg | obj <- symObjs]
   in outsetBox r $ intersectBoxes boxes
-- FIXME: implement Rotate2.
-- Fallthrough: rotate the points of the containing box. no rounding.
getBox2R symObj deg =
  let origBox = getBox2 symObj
      points = corners origBox
   in getBox2R (Polygon points) deg
...
pointRBox (V2 xStart yStart) travel =
  let k :: ℝ
      ...
      rotationDirection = case travel of
        polarity
          | polarity > 0 -> Rotation CounterClockwise
          | polarity == 0 -> None
      ...
      crossOne start dir
        | (start == PosX && dir == Clockwise)
            || (start == NegX && dir == CounterClockwise) =
          mixWith [(0, - distance)]
        | (start == NegY && dir == Clockwise)
            || (start == PosY && dir == CounterClockwise) =
          mixWith [(- distance, 0)]
        | (start == NegX && dir == Clockwise)
            || (start == PosX && dir == CounterClockwise) =
          mixWith [(0, distance)]
        | (start == PosY && dir == Clockwise)
            || (start == NegY && dir == CounterClockwise) =
          mixWith [(distance, 0)]
        | otherwise = distanceBox
