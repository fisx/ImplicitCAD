...
import Linear (V2 (V2))
import Prelude (Eq, abs, cos, cycle, drop, filter, fmap, fst, length, minimum, odd, sin, sqrt, uncurry, zip, ($), (&&), (*), (+), (-), (.), (/), (/=), (<), (<=), (>), (>=), (||))
...
getImplicit2 _ (Polygon (scanUniqueCircular -> points@(_ : _ : _ : _))) =
  \p ->
    let pairs :: [(ℝ2, ℝ2)]
        ...
        crossing_points =
          [ x2 - y2 * (x2 - x1) / (y2 - y1) | (V2 x1 y1, V2 x2 y2) <- relativePairs, ((y2 <= 0) && (y1 >= 0)) || ((y2 >= 0) && (y1 <= 0))
          ]
     ...
     in minimum dists * if isIn then -1 else 1
getImplicit2 ctx (Polygon _) = getImplicitShared @SymbolicObj2 ctx Empty
-- (Rounded) CSG
getImplicit2 ctx (Rotate2 θ symbObj) =
  \(V2 x y) ->
    let obj = getImplicit2 ctx symbObj
     in obj $ V2 (x * cos θ + y * sin θ) (y * cos θ - x * sin θ)
