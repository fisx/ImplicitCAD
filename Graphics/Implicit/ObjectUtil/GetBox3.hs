...
import Graphics.Implicit.Definitions
  ( Box3,
    ExtrudeMScale (C1, C2),
    Fastℕ,
    SymbolicObj3 (Cube, Cylinder, Extrude, ExtrudeM, ExtrudeOnEdgeOf, Rotate3, RotateExtrude, Shared3, Sphere),
    fromFastℕ,
    fromFastℕtoℝ,
    toScaleFn,
    ℝ,
...
import Graphics.Implicit.ObjectUtil.GetBoxShared (corners, getBoxShared, pointsBox)
import Linear (V2 (V2), V3 (V3))
import qualified Linear.Quaternion as Q
import Prelude (Bool (False), Either (Left, Right), abs, const, either, fmap, fst, max, maximum, min, minimum, otherwise, pure, snd, take, uncurry, unzip, ($), (*), (+), (-), (.), (/), (<), (<$>), (==), (>))
...
getBox3 (ExtrudeOnEdgeOf symbObj1 symbObj2) =
  let (V2 ax1 ay1, V2 ax2 ay2) = getBox2 symbObj1
      (V2 bx1 by1, V2 bx2 by2) = getBox2 symbObj2
   in (V3 (bx1 + ax1) (by1 + ax1) ay1, V3 (bx2 + ax2) (by2 + ax2) ay2)
...
getBox3 (ExtrudeM twist scale translate symbObj height) =
  let (V2 x1 y1, V2 x2 y2) = getBox2 symbObj
      (dx, dy) = (x2 - x1, y2 - y1)
   ...
   in (V3 (twistXmin + tminx) (twistYmin + tminy) 0, V3 (twistXmax + tmaxx) (twistYmax + tmaxy) h)
...
getBox3 (RotateExtrude _ (Left (V2 xshift yshift)) _ symbObj) =
  let (V2 _ y1, V2 x2 y2) = getBox2 symbObj
      r = max x2 (x2 + xshift)
   in (V3 (- r) (- r) $ min y1 (y1 + yshift), V3 r r $ max y2 (y2 + yshift))
...
getBox3 (RotateExtrude rot (Right f) rotate symbObj) =
  let samples :: Fastℕ
      ...
      (r, _, _) =
        if either (== 0) (const False) rotate
          then
            let s = maximum $ fmap abs [x2, y1, y2]
             in (s + xmax', s + ymin', y2 + ymax')
          else (x2 + xmax', y1 + ymin', y2 + ymax')
   in (V3 (- r) (- r) $ y1 + ymin', V3 r r $ y2 + ymax')
