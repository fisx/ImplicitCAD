...
import Graphics.Implicit.Definitions (NormedTriangleMesh (NormedTriangleMesh), Polyline, SymbolicObj2, SymbolicObj3, Triangle, TriangleMesh (TriangleMesh), defaultObjectContext, ℝ, ℝ2)
import Graphics.Implicit.Export.RayTrace (Camera (Camera), Color (Color), Light (Light), Scene (Scene), average, cameraRay, traceRay)
...
import Prelude (Int, abs, fromIntegral, max, maximum, pure, realToFrac, round, ($), (*), (+), (-), (/), (<))
...
instance DiscreteAproxable SymbolicObj3 DynamicImage where
  discreteAprox _ symbObj = ImageRGBA8 $ generateImage pixelRenderer (round w) (round h)
    where
      ...
      pixelRenderer a b =
        renderScreen
          (fromIntegral a / w - 0.5)
          (fromIntegral b / h - 0.5)
