...
import Graphics.Implicit.Definitions (Fastℕ, Obj3, ℝ, ℝ2, ℝ3)
import Linear
  ( Metric (dot, norm),
    V2 (V2),
    V3 (V3),
...
import Prelude (Bool (False, True), Maybe (Just, Nothing), RealFrac, Show, abs, flip, floor, fmap, fromInteger, fromIntegral, length, max, maximum, min, minimum, not, otherwise, pred, pure, round, sum, toRational, unzip, ($), (*), (+), (-), (.), (/), (<), (==), (>))
...
rayBounds ray box =
  let Ray (V3 cPx cPy cPz) (V3 cVx cVy cVz) = ray
              ...
              ( sum $
                  0.2 : do
                    ...
                    let pval = obj p
                        ...
                        rV =
                          let normalComponent = proj v' normal
                              parComponent = v' - normalComponent
                           in normalComponent - parComponent
