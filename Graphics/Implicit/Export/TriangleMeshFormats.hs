...
import Blaze.ByteString.Builder (Write, fromByteString, fromWord16le, fromWord32le, fromWrite, toLazyByteString, writeStorable)
...
import Graphics.Implicit.Definitions (Triangle (Triangle), TriangleMesh (TriangleMesh), fromℝtoFloat, ℕ, ℝ, ℝ3)
import Graphics.Implicit.Export.TextBuilderUtils (Builder, Text, bf, buildℕ, toLazyText)
import Linear (V3 (V3), cross, normalize)
import Prelude (Bool, Eq, Float, filter, length, not, pure, toEnum, zip, ($), (&&), (+), (-), (.), (<>), (==), (||))
...
cleanupTris tris =
  let floatPoint :: V3 ℝ -> (Float, Float, Float)
...
binaryStl triangles = toLazyByteString $ header <> lengthField <> foldMap triangle (unmesh $ cleanupTris triangles)
  where
    header = fromByteString $ replicate 80 0
...
jsTHREE triangles = toLazyText $ header <> vertcode <> facecode <> footer
  where
    ...
    facecode = fold $ do
      ...
      let (posa, posb, posc) = (n, n + 1, n + 2) :: (ℕ, ℕ, ℕ)
