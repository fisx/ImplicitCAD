...
import Graphics.Implicit.Definitions (Triangle (Triangle), TriangleMesh (TriangleMesh))
import Graphics.Implicit.Export.Render.Definitions (TriSquare (Sq, Tris))
import Linear (V2 (V2), (*^), (^*))
import Prelude (compare, concat, concatMap, error, fmap, foldMap, otherwise, ($), (+), (.), (<>), (==))
...
mergedSquareTris sqTris =
  let -- We don't need to do any work on triangles. They'll just be part of
      ...
      joined =
        fmap
          ( concatMap joinXaligned . groupWith (\(Sq _ _ xS _) -> xS)
              . concatMap joinYaligned
              . groupWith (\(Sq _ _ _ yS) -> yS)
              . concatMap joinXaligned
              . groupWith (\(Sq _ _ xS _) -> xS)
          )
          planeAligned
      -- Merge them back together, and we have the desired reult!
      finishedSquares = concat joined
   in -- merge them to triangles, and combine with the original triangles.
...
squareToTri (Sq (b1, b2, b3) z (V2 x1 x2) (V2 y1 y2)) =
  let zV = b3 ^* z
      ...
      c = zV + x1V + y2V
      d = zV + x2V + y2V
   in [Triangle (a, b, c), Triangle (c, b, d)]
