...
import Graphics.Implicit.Definitions (Polyline (Polyline), minℝ)
import Linear (V2 (V2))
import Prelude (Maybe (Just, Nothing), abs, error, fmap, last, otherwise, reverse, ($), (&&), (*), (-), (.), (/), (<=), (<>), (==), (>), (||))
...
joinSegs (Polyline present : remaining) =
  let findNext :: [Polyline] -> (Maybe Polyline, [Polyline])
