...
import Graphics.Implicit.Definitions (Fastℕ, Obj2, Polyline (Polyline), minℝ, ℝ, ℝ2)
import Graphics.Implicit.Export.Util (centroid)
import Linear (Metric (dot, norm), V2 (V2), normalize, (^*))
...
detail n res obj (Polyline [p1, p2])
  | n < 2 =
    let mid = centroid [p1, p2]
        midval = obj mid
     in if abs midval < res / 40
          then Polyline [p1, p2]
          else
            let normal = (\(V2 a b) -> V2 b (- a)) $ normalize (p2 - p1)
                derivN = - (obj (mid - (normal ^* (midval / 2))) - midval) * (2 / midval)
             in if abs derivN > 0.5 && abs derivN < 2 && abs (midval / derivN) < 3 * res
                  then
                    let mid' = mid - (normal ^* (midval / derivN))
                     in addPolylines (detail (n + 1) res obj (Polyline [p1, mid'])) (detail (n + 1) res obj (Polyline [mid', p2]))
                  else -- NOTE: we used to have a routine for increasing the sharpness of corners here, but it was too buggy. - JEL
                    Polyline [p1, p2]
detail _ _ _ x = x
