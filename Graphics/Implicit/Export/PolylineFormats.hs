...
import Graphics.Implicit.Export.TextBuilderUtils (Builder, Text, bf, buildInt, buildTruncFloat, toLazyText)
import Linear (V2 (V2))
import Text.Blaze.Internal (stringValue)
import Text.Blaze.Svg.Renderer.Text (renderSvg)
import Text.Blaze.Svg11 (Svg, docTypeSvg, g, polyline, toValue, (!))
import qualified Text.Blaze.Svg11.Attributes as A (fill, height, points, stroke, strokeWidth, version, viewbox, width)
import Prelude (compare, fmap, foldl, length, max, maximum, mempty, min, minimum, show, snd, unwords, unzip, ($), (+), (-), (.), (/), (<$>), (<>))
...
svg plines = renderSvg . svg11 . svg' $ plines
  where
    ...
    poly (Polyline line) = polyline ! A.points pointList
      where
        pointList = toValue $ toLazyText $ fold [bf (x - xmin) <> "," <> bf (ymax - y) <> " " | (V2 x y) <- line]
