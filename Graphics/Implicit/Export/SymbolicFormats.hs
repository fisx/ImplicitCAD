...
import Graphics.Implicit.Definitions (SharedObj (Complement, DifferenceR, EmbedBoxedObj, Empty, Full, IntersectR, Mirror, Outset, Scale, Shell, Translate, UnionR, WithRounding), SymbolicObj2 (Circle, Polygon, Rotate2, Shared2, Square), SymbolicObj3 (Cube, Cylinder, Extrude, ExtrudeM, ExtrudeOnEdgeOf, Rotate3, RotateExtrude, Shared3, Sphere), isScaleID, quaternionToEuler, ℝ)
import Graphics.Implicit.Export.TextBuilderUtils (Builder, Text, bf, fromLazyText, toLazyText)
import Graphics.Implicit.ObjectUtil.GetBoxShared (VectorStuff (elements))
import Linear (V2 (V2), V3 (V3))
import Prelude (Either (Left, Right), const, error, floor, fmap, pi, pure, sequenceA, take, ($), ($!), (&&), (*), (+), (-), (.), (/), (<$>), (<>), (==))
...
buildS3 (ExtrudeM twist scale (Left translate) obj (Left height)) | isScaleID scale && translate == V2 0 0 = do
  ...
  call
    ...
    [ call
        ...
        [ callNaked
            "linear_extrude"
            ["height = " <> bf res, "twist = " <> bf (twist' (h + res) - twist' h)]
            [ buildS2 obj
            ]
        ]
