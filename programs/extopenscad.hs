...
import Graphics.Implicit (runOpenscad, unionR, writeBinSTL, writeDXF2, writeGCodeHacklabLaser, writeOBJ, writePNG2, writePNG3, writeSCAD2, writeSCAD3, writeSTL, writeSVG)
...
import Options.Applicative (Parser, argument, auto, execParser, footer, fullDesc, header, help, helper, info, long, metavar, option, optional, short, str, strOption, switch)
import System.FilePath (splitExtension)
import System.IO (Handle, IOMode (WriteMode), hPutStr, openFile, stderr, stdout)
import Prelude (Bool (False, True), Eq, FilePath, IO, Maybe (Just, Nothing), Read (readsPrec), Show, String, drop, error, filter, fst, length, lookup, min, minimum, not, null, print, putStrLn, readFile, return, show, sqrt, tail, take, unlines, ($), (&&), (*), (**), (-), (.), (/), (<>), (==), (>), (>>=), (||))
...
run rawargs = do
  ...
  case (obj2s, obj3s) of
    ([], obj : objs) -> do
      ...
      if null objs
        then return ()
        else hPutStr stderr "WARNING: Multiple objects detected. Adding a Union around them.\n"
